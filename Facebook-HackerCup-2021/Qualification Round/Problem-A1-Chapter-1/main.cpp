...
* @Modified Time: 2021-10-17 07:56:16  (IST)
...
void solve()
{
	...
	for (int i = 0; i < 26; i++) {
		if (a[i] > 0) {
			...
			for (int j = 0; j < 26; j++) {
				if (i != j and a[j] > 0) {
					if (ok(char(i + 'A')) == true && ok(char(j + 'A')) == true)
					{
						sm += (2 * a[j]);
					}
					else if (ok(char(i + 'A')) == true && ok(char(j + 'A')) == false)
					{
						sm += a[j];
					}
					else if (ok(char(i + 'A')) == false && ok(char(j + 'A')) == false)
					{
						sm += (2 * a[j]);
					}
					else if (ok(char(i + 'A')) == false && ok(char(j + 'A')) == true)
					{
						sm += a[j];
					}
				}
